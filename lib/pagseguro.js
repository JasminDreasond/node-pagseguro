      ...
      function (resolve, reject) {
        ...
        req(options, function (err, res, body) {
          ...
          if (err) {
            ...
            // Convert XML
            body = JSON.parse(xmlConvert.xml2json(body));
            ...
            if (errors.length < 1) {
              ...
              let checkout = {};
              for (const item in body.elements) {
                if (body.elements[item].name === "checkout") {
                  for (const item2 in body.elements[item].elements) {
              ...
              checkout.codeURL = tinythis.uri + tinythis.version + '/checkout/payment.html?code=' + checkout.code;
