...
class pagseguro {
  ...
  addreceiver(split, publicKey) {
    ...
    if (typeof this.obj.receiver === "undefined") {
      this.obj.receiver = [];
    }
  ...
  send() {
    ...
    switch (this.mode) {
      case 'payment':
        options.uri = this.uri + this.version + "/checkout" + this.credentialsURL;
        options.body = this.xml + xml.toXML({
          checkout: this.obj
        });
        break;
      case 'subscription':
        options.uri = this.uri + this.version + "/pre-approvals/request" + this.credentialsURL;
        options.body = this.xml + xml.toXML({
  ...
  setAppMode(enabled, code = '') {
    ...
    if (!enabled) {
      this.credentialsURL = "?email=" + this.email + "&token=" + this.token;
    } else {
      if (code) {
        this.credentialsURL = "?appId=" + this.email + "&appKey=" + this.token + "&authorizationCode=" + code;
      } else {
        this.credentialsURL = "?appId=" + this.email + "&appKey=" + this.token;
      }
    }
