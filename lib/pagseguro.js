...
class pagseguro {
  ...
  addreceiver(split, publicKey, isEmail) {
  ...
  cancelPayment(code) {
    ...
    options.body = 'transactionCode=' + code;

    options.uri = this.uri + this.version + "/transactions/cancels" + this.getAppMode();
  ...
  refundPayment(code, refund) {
    ...
    if (typeof refund !== "undefined") {
      options.body = 'transactionCode=' + code + '&refundValue=' + refund;
    } else {
      options.body = 'transactionCode=' + code;
    }

    options.uri = this.uri + this.version + "/transactions/refunds" + this.getAppMode();
  ...
  getAppMode(json) {
    ...
    if (!json) {
      ...
      if (!this.appMode) {
        ...
        if (this.appAuthorization) {
          result.authorizationCode = this.appAuthorization;
        }
      }
